...
// 		문자 (한문자) : 'ㅁ'	char ch = 'a'
		...
		/*
		 ...
		 4. 작은 타입에는 큰 타입을 넣고싶다면 강제로 (casting)
		 책임은 우리가.
		 char << int 강제로 char << (char)int 데이터 손실 발생 가능. 
		 int << char 내부적으로 컴파일러가 친절하게 int << (int)char 처리함.
		 */
		
		//String 문자열을 담을 수 있는 타입
		//쓰는법 : int, double 처럼 사용.
		...
		// + >> 산술연산, 결합 연산
		//tip) Oracle 데이터베이스(언어) > + 산술연산, || 결합연산자 (ex '안녕' ||'방가')
		
		//tip) 자바에서 특수문자 처리.
		//이스케이프문자 : 특정문자 앞에 \ 붙이기.
		char sing ='\''; // \통해서 ' 데이터임을 표현
		...
		//실수형 타입 (부동소수점)
		//float 4byte
		//double 8byte //실수 리터럴 기본값은 double이다.
		
		float f = 3.14f;
		//리터럴 값이 기본 double이기 때문에
		// float f = 3.14; 로는 문제가 남
		// 값이 작으면 상관이 없지만 이런걸 위해 double로 쓰는게 맞음.
		
//		float f = 3.14; //현명한 선택(담을 그릇을 크게); > double로 바꿈.
